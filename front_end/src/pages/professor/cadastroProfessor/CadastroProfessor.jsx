...
const CadastroProfessor = () => {
  ...
  // Descobrir se quem está acessando a tela é um Admin logado
  const isAdmin = () => {
    const token = localStorage.getItem("meu_token_tg");
    if (token) {
      try {
        const payload = jwtDecode(token);
        const cargosToken = payload.cargos || [];
        return cargosToken.includes("ROLE_ADMIN");
      } catch (e) {
        console.error(e)
        return false;
      }
  ...
  const enviarParaBackend = async (dadosValidados) => {
    try {
      ...
      // Se for Admin cadastrando outro professor
      if (isAdmin()) {
        setResultado({
          exibir: true,
          variante: "success",
      ...
      // Se for o próprio professor se cadastrando solto na internet
        ...
        setTimeout(() => {
          navigate("/confirmar-email", { state: { emailCapturado: dadosValidados.email } });
        }, 2000);
      }
